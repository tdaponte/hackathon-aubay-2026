...
try:
    # Evite l'affichage charabia des accents dans les terminaux dont
    # l'encodage par défaut n'est pas l'UTF-8 (typiquement Windows/cp1252).
    sys.stdout.reconfigure(encoding="utf-8")
except AttributeError:
    pass  # stdout ne supporte pas reconfigure (flux redirigé, etc.)
...
def collect_and_fill_form(llm, ask_user_fn) -> dict:
    """
    :param ask_user_fn: fonction qui prend (question: str, field_info: dict)
        et retourne la réponse de l'utilisateur (str).
        - En CLI : `lambda q, field_info: input(q + " ")`
        - Avec le front Streamlit : `bridge.ask_user_via_streamlit`
    """
    ...
    # Force l'UTF-8 : sans ça, `requests` devine parfois l'encodage à tort
    # (ex: latin-1) quand le content-type ne précise pas de charset, ce qui
    # abîme les accents dès la source (labels/descriptions du formulaire).
    ...
    while not state.is_complete:
        ...
        user_reply = ask_user_fn(question, field_info)
...
if __name__ == "__main__":
    from bridge import ask_user_via_streamlit

    # Lance ce script dans un terminal, et `streamlit run interface_streamlit.py`
    # dans un autre. Les deux communiquent via current_field.json /
    # current_response.json dans le même dossier.
    answers = collect_and_fill_form(llm, ask_user_fn=ask_user_via_streamlit)
